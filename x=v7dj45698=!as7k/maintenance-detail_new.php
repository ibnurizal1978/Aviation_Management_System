        ...
        <div class="block table-responsive">
            ...
            <div class="block-content">
                ...
                <table class="table table-sm table-vcenter">
                    ...
                    <tbody>
                        <?php
                        $sql2   = "SELECT  aircraft_parts_id,item_number,description,part_number,serial_number, date_format(installed_date, '%d/%m/%Y') as installed_date,ldg,mth,hrs FROM tbl_aircraft_parts WHERE  (mth<>0 OR hrs<>0 or ldg<>0) AND aircraft_master_id = '".$row['aircraft_master_id']."' ORDER by description";
                        ...
                        <tr>
                            ...
                            <?php
                            //date
                            $sql3   = "SELECT DATEDIFF(next_due,CURDATE()) AS selisih, date_format(installed_date, '%d/%m/%Y') as installed_date,date_format(next_due, '%d/%m/%Y') as next_due,ldg,mth,hrs FROM tbl_aircraft_parts WHERE installed_date <> '0000-00-00' AND DATEDIFF(next_due, CURDATE()) < 90 AND mth<>0 AND aircraft_parts_id = '".$row2['aircraft_parts_id']."' ORDER by selisih,next_due";
                            $h3      = mysqli_query($conn,$sql3);
                            $row3 = mysqli_fetch_assoc($h3);
                            if (date('d/m/Y') > $row3['next_due']) {
                                echo '<font class=text-danger>'.$row3['next_due'].'</font>';
                            }else{
                                echo '<font class=text-success>'.$row3['next_due'].'</font>';
                            }
                            //end date

                            //ldg
                            $sql3   = "SELECT DATEDIFF(next_due,CURDATE()) AS selisih, date_format(installed_date, '%d/%m/%Y') as installed_date,date_format(next_due, '%d/%m/%Y') as next_due,ldg,mth,hrs FROM tbl_aircraft_parts WHERE ldg <> 0 AND ldg < '".$row['aircraft_total_ldg']."' AND aircraft_parts_id = '".$row2['aircraft_parts_id']."'";
                            $h3      = mysqli_query($conn,$sql3);
                            $row3 = mysqli_fetch_assoc($h3);
                            if ($row['aircraft_total_ldg'] > $row3['ldg']) {
                                echo '<font class=text-danger>'.$row3['ldg'].'</font>';
                            }else{
                                echo '<font class=text-success>'.$row3['ldg'].'</font>';
                            }
                            //end ldg                            
                            ?>                                                                                        
                            </td>
