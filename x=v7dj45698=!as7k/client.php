...
<main id="main-container">
    ...
    <!-- Small Table -->
      ...
      if(@$_REQUEST['s']=='1091vdf8ame151') {
        ...
        $sql = "SELECT *,date_format(client_expiry_date, '%d/%m/%Y') as client_expiry_date FROM tbl_client WHERE (client_name LIKE '%$txt_search%' OR client_code LIKE '%$txt_search%' OR client_email_address LIKE '%$txt_search%') LIMIT $offset, $dataPerPage";
      }elseif (@$_REQUEST['s']=='1'){
        $sql = "SELECT *,date_format(client_expiry_date, '%d/%m/%Y') as client_expiry_date FROM tbl_client ORDER by client_name LIMIT $offset, $dataPerPage";      
      }else{
        $sql = "SELECT *,date_format(client_expiry_date, '%d/%m/%Y') as client_expiry_date FROM tbl_client LIMIT $offset, $dataPerPage";
      }
      ...
      ?>    
                ...
                <table class="table table-sm table-vcenter">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><?php echo $row['client_expiry_date'] ?></td>
                            <td>
                            ...
                            <td class="text-center">
                                <div class="btn-group">
                                    <a type="button" class="btn btn-sm btn-secondary" data-toggle="tooltip" title="Edit" href="client-detail.php?act=29dvi59&ntf=29dvi59-<?php echo $row["client_id"]?>-94dfvj!sdf-349ffuaw">
        ...
        if(@$_REQUEST['s']=='1091vdf8ame151') {           
          $sql = "SELECT count(client_id) as jumData FROM tbl_client WHERE (client_name LIKE '%$txt_search%' OR client_code LIKE '%$txt_search%' OR client_email_address LIKE '%$txt_search%')";
        }elseif(@$_REQUEST['s']=='1') {           
          $sql = "SELECT count(client_id) as jumData FROM tbl_client ORDER by client_name";
        }else{            
          $sql = "SELECT count(client_id) as jumData FROM tbl_client";
        }          
