...
$afml_source   = input_data(filter_var($_POST['afml_source'],FILTER_SANITIZE_STRING));
$afml_fuel_location   = input_data(filter_var($_POST['afml_fuel_location'],FILTER_SANITIZE_STRING));

if($afml_fuel_location == "") {
  header('location:afml-detail-new.php?act=79dvi59g&ntf=29dvi59-'.$afml_id.'-94dfvj!sdf-349ffuaw');
  exit(); 
}

if($afml_source <> "DRUM" && $afml_receipt_no == "") {
  header('location:afml-detail-new.php?act=7k0dvi59&ntf=29dvi59-'.$afml_id.'-94dfvj!sdf-349ffuaw');
  exit(); 
}

if($afml_receipt_no <> "" && $afml_fuel_date == "") {
  header('location:afml-detail-new.php?act=70dvi59&ntf=29dvi59-'.$afml_id.'-94dfvj!sdf-349ffuaw');
  exit(); 
}

if($afml_source == "PERTAMINA" && @$_FILES['upload_file']['name'] == "") {
  header('location:afml-detail-new.php?act=7k0dvi59&ntf=29dvi59-'.$afml_id.'-94dfvj!sdf-349ffuaw');
  exit(); 
}
...
//apakah ada duplikat?
$sql  = "SELECT afml_receipt_no FROM tbl_afml_detail_old WHERE client_id = '".$_SESSION['client_id']."' AND afml_receipt_no<>'' AND afml_receipt_no = '".$afml_receipt_no."' LIMIT 1";
$h    = mysqli_query($conn,$sql);
if(mysqli_num_rows($h)>0) {
  header('location:afml-detail-new.php?act=79dvi59&ntf=29dvi59-'.$afml_id.'-94dfvj!sdf-349ffuaw');
  exit(); 
}
...
if(@$_FILES['upload_file']['name'] != ''){
  ...
  @$newfilename = $afml_receipt_no.'-'.date('dmY').'.'.end($temp);
  $target_file = $target_dir.$newfilename;
  $imageFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));

  //file harus < 1MB
  if ($_FILES["upload_file"]["size"] > 1000001) {
  header('location:afml-detail-new.php?act=09dvi59&ntf=29dvi59-'.$afml_id.'-94dfvj!sdf-349ffuaw');
  exit();
  }
...
move_uploaded_file($_FILES["upload_file"]["tmp_name"], $target_file);
  ...
  $sql   = "UPDATE tbl_afml_detail_old SET afml_fuel_rem = '".$afml_fuel_rem."',afml_fuel_uplift = '".$afml_fuel_uplift."', afml_fuel_total= '".$afml_fuel_total."', afml_added_oil = '".$afml_added_oil."', afml_added_hyd = '".$afml_added_hyd."', afml_receipt_no = '".$afml_receipt_no."', afml_fuel_date = '".$afml_fuel_date_f."', engineer_created_date = UTC_TIMESTAMP(), engineer_user_id = '".$_SESSION['user_id']."' WHERE afml_detail_id = '".$afml_detail_id."' LIMIT 1";
}
...
header('location:afml-detail-new.php?act=a29dvi59&ntf=29dvi59-'.$afml_id.'-94dfvj!sdf-349ffuaw');
?>