...
<main id="main-container">
    ...
    <?php
      ...
      if(@$_REQUEST['s']=='1091vdf8ame151') {
        ...
        $sql = "SELECT * FROM tbl_aircraft_master a INNER JOIN tbl_aircraft_type b USING (aircraft_type_id) WHERE (aircraft_reg_no LIKE '%$txt_search%' OR aircraft_reg_code LIKE '%$txt_search%' OR aircraft_type LIKE '%$txt_search%') AND a.client_id = '".$_SESSION['client_id']."' LIMIT $offset, $dataPerPage";
      }elseif (@$_REQUEST['s']=='1'){
        $sql = "SELECT * FROM tbl_aircraft_master a INNER JOIN tbl_aircraft_type b USING (aircraft_type_id) WHERE a.client_id = '".$_SESSION['client_id']."' ORDER by aircraft_reg_code LIMIT $offset, $dataPerPage";      
      }else{
        $sql = "SELECT * FROM tbl_aircraft_master a INNER JOIN tbl_aircraft_type b USING (aircraft_type_id) WHERE a.client_id = '".$_SESSION['client_id']."' LIMIT $offset, $dataPerPage";
      }
    ...
    <?php } ?>    
        <div class="block table-responsive">
            <div class="block-header block-header-default">
                <h3 class="block-title">Aircraft</h3>
                <div class="block-options">
        ...
        if(@$_REQUEST['s']=='1091vdf8ame151') {           
          $sql = "SELECT count(aircraft_master_id) as jumData FROM tbl_aircraft_master a INNER JOIN tbl_aircraft_type b USING (client_id) WHERE a.client_id = '".$_SESSION['client_id']."' AND (aircraft_type_id) WHERE (aircraft_reg_no LIKE '%$txt_search%' OR aircraft_reg_code LIKE '%$txt_search%' OR aircraft_type LIKE '%$txt_search%')";
        }elseif(@$_REQUEST['s']=='1') {           
          $sql = "SELECT count(aircraft_master_id) as jumData FROM tbl_aircraft_master a INNER JOIN tbl_aircraft_type b USING (client_id) WHERE b.client_id = '".$_SESSION['client_id']."' ORDER by aircraft_reg_code";
        }else{            
          $sql = "SELECT count(aircraft_master_id) as jumData FROM tbl_aircraft_master a INNER JOIN tbl_aircraft_type x USING (client_id) WHERE a.client_id = '".$_SESSION['client_id']."'";
        }          
