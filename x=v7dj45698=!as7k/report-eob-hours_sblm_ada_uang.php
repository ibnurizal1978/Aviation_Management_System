                ...
                <div class="block block-rounded">
                    <div class="block-content">
                        <table class="table table-borderless table-striped">
                            ...
                            <tbody>
                                <?php
                                $sql  = "SELECT user_id,full_name FROM tbl_user WHERE department_id = 1 AND user_active_status = 1 AND user_id NOT IN ('35') AND user_position LIKE '%engineer%' AND client_id = '".$_SESSION['client_id']."'";
                                $h    = mysqli_query($conn,$sql);
                                while($row = mysqli_fetch_assoc($h)) {
                                
                                /*-------------------------------------------------------*/

                                    //hitung hrs untuk bulan ini
                                    $sql_tot = "SELECT SUM(a.afml_block_hrs) AS total_block_hrs, SUM(a.afml_flt_hrs) AS total_flt_hrs FROM tbl_afml_detail a INNER JOIN tbl_afml b USING (afml_page_no) WHERE b.afml_engineer_on_board_user_id = '".$row['user_id']."' AND month(a.afml_date) = month(NOW())  AND year(a.afml_date) = year(NOW()) AND a.client_id = '".$_SESSION['client_id']."'";
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <a href="report-eob-hours-detail.php?act=29dvi59&ntf=29dvi59-<?php echo $row["user_id"]?>-94dfvj!sdf-349ffuaw">Detail</a>
                                    </td>                                    
                                </tr>
                            <?php } ?>
                            </tbody>
