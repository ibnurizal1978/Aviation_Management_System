                        ...
                        <table class="table table-borderless table-striped">
                            ...
                            <tbody>
                                ...
                                while($row = mysqli_fetch_assoc($h)) {
                                    ...
                                    $sql_tot = "SELECT SUM(a.afml_block_hrs) AS total_block_hrs, SUM(a.afml_flt_hrs) AS total_flt_hrs, sum(b.total_price) as total_price FROM tbl_afml_detail a INNER JOIN tbl_afml_price b USING (afml_page_no) WHERE (b.afml_captain_user_id = '".$row['user_id']."' OR b.afml_copilot_user_id = '".$row['user_id']."') AND month(a.afml_date) = month(NOW())  AND year(a.afml_date) = year(NOW()) AND a.client_id = '".$_SESSION['client_id']."'";
                                    echo $sql_tot;
                                ...
                                <tr>
                                    ...
                                    <td><?php echo $row['full_name']. '(Rp. '.number_format($row_tot['total_price'],2,",",".").')' ?></td> 
                                    <td class="text-right">
